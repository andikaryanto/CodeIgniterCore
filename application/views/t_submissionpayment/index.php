              ...
              <div class="card">
                  ...
                  <div class="row">
                    ...
                    <div class="col">
                      <div class="text-right">
                        <button type="button" rel="tooltip" class="btn btn-primary btn-round btn-fab" title="<?= lang('ui_add')?>" onclick="window.location.href='<?= base_url('tsubmissionpayment/add');?>'">
                ...
                <div class="card-body">
                  ...
                  <div class="material-datatables">
                    <div>
                      Toggle column: <a href="#" class="toggle-vis text-primary" data-column="0"><?=  lang('ui_nopayment')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="1"><?=  lang('ui_paymentdate')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="2"><?=  lang('ui_nosubmission')?></a> -
                                      <a href="#" class="toggle-vis text-primary" data-column="3"><?=  lang('ui_amountpayment')?></a> -
                          ...
                          <div class="table-responsive">
                            <table data-page-length="<?= $_SESSION['usersettings']['RowPerpage']?>" id = "tablePeople" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info">
                              <thead class=" text-primary">
                                <tr role = "row">
                                  <!-- <th># </th> -->
                                  <th><?=  lang('ui_nopayment')?></th>
                                  <th><?=  lang('ui_paymentdate')?></th>
                                  <th><?=  lang('ui_nosubmission')?></th>
                                  <th><?=  lang('ui_amountpayment')?></th>
                              ...
                              ?>
                                  <tr role = "row" id = <?= $value->Id?>>
                                    <td><?= $value->NoPayment?></td>
                                    <td><?= get_formated_date($value->PaymentDate, 'd-m-Y')?></td>
                                    <td><?= $value->get_T_Submission()->NoSubmission ?></td>
                                    <td><?= number_format($value->getAmountPayment(), 2, ",", ".")?></td>
                                    <td><?= $value->Created?></td>
  ...
  function dataTable(){
    ...
    $('a.toggle-vis').on( 'click', function (e) {
     ...
     table.on( 'click', '.edit', function () {
        ...
        var id = $tr.attr('id');
        window.location = "<?= base_url('tsubmissionpayment/edit/');?>" + id;
     } );
     ...
     table.on( 'click', '.delete', function (e) {
        ...
        deleteData(data[0], function(result){
          if (result==true)
          {
            ...
            $.ajax({
              type : "POST",
              url : "<?= base_url('tsubmissionpayment/delete/');?>",
              data : {id : id},
