              ...
              <div class="card">
                  ...
                  <div class="row">
                    ...
                    <div class="col">
                      <div class="text-right">
                        <button type="button" rel="tooltip" class="btn btn-primary btn-round btn-fab" title="<?= lang('ui_add')?>" onclick="window.location.href='<?= base_url('mworker/add');?>'">
                ...
                <div class="card-body">
                  ...
                  <div class="material-datatables">
                    <div>
                      Toggle column: <a href="#" class="toggle-vis text-primary" data-column="0"><?=  lang('ui_number')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="1"><?=  lang('ui_name')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="2"><?=  lang('ui_telephone')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="3"><?=  lang('ui_email')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="4"><?=  lang('ui_address')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="5"><?=  lang('ui_type')?></a> - 
                                      <a href="#" class="toggle-vis text-primary" data-column="6"><?=  lang('ui_createat')?></a>
                    </div>
                            ...
                            <table data-page-length="<?= $_SESSION['usersettings']['RowPerpage']?>" id = "tablePeople" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed" cellspacing="0" width="100%" style="width: 100%;" role="grid" aria-describedby="datatables_info">
                              ...
                              ?>
                                  <tr role = "row" id = <?= $value->Id?>>
                                    <td><?= $value->NoWorker?></td>
                                    <td><?= $value->get_M_People()->CompleteName?></td>
                                    <td><?= $value->Phone?></td>
                                    <td><?= $value->Email?></td>
                                    <td><?= $value->get_M_People()->Address?></td>
                                    <td><?= getEnumName("Worker", $value->WorkerType)?></td>
                                    <td><?= $value->Created?></td>
...
<script>
  ...
  function dataTable(){
    ...
    for (index = 0; index < datacolumn.length; ++index) {
      ...
      if(data.getAttribute("data-column") == 4 || 
    ...
    $('a.toggle-vis').on( 'click', function (e) {
     ...
     table.on( 'click', '.edit', function () {
        ...
        var id = $tr.attr('id');
        window.location = "<?= base_url('mworker/edit/');?>" + id;
     } );
